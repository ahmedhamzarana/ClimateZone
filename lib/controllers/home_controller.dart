...
class HomeController extends GetxController {
  ...
  void listenToRealtimeData() {
    currentRef.onValue.listen((DatabaseEvent event) {
      ...
      humidity.value =
          double.tryParse(values['humidity'].toString()) ?? 0;

      gasValue.value =
          double.tryParse(values['gasValue'].toString()) ?? 0;

      status.value =
          values['status']?.toString() ?? 'UNKNOWN';

      dhtStatus.value =
          values['dht']?.toString() ?? 'UNKNOWN';

      uptime.value =
          int.tryParse(values['uptime'].toString()) ?? 0;
    });
  }
    ...
    historyRef
      ...
      values.forEach((key, value) {
        ...
        result.add({
          ...
          'humidity':
              double.tryParse(item['humidity'].toString()) ?? 0,

          'gasValue':
              double.tryParse(item['gasValue'].toString()) ?? 0,

          'status':
              item['status']?.toString() ?? 'UNKNOWN',

          'dht':
              item['dht']?.toString() ?? 'UNKNOWN',

          'uptime':
              int.tryParse(item['uptime'].toString()) ?? 0,
        });
      });
